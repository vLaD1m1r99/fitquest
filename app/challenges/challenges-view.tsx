...
import { type ChallengesData, type RPG } from "@/lib/data"
...
export function ChallengesView({ challengesData, rpg }: ChallengesViewProps) {
	...
	const rewardLevels = Object.keys(challengesData.levelRewards)
		.map(Number)
		.sort((a, b) => a - b)
	const currentRewardLevel = rewardLevels.filter(l => l <= rpg.level).pop()
	const currentReward = currentRewardLevel ? challengesData.levelRewards[String(currentRewardLevel)] : null

	const monthlyData = challengesData.monthlyChallenges
		...
		<div className="space-y-8">
			...
			<div>
				...
				<div className="grid gap-4 md:grid-cols-2">
					{challengesData.activeChallenges.map(ch => {
				...
				<Card className="p-6">
					<div className="space-y-3">
						{rewardLevels.map(lvl => {
							const reward = challengesData.levelRewards[String(lvl)]
			...
			{challengesData.completedChallenges.length > 0 && (
				<div>
					...
					{showCompleted && (
						<div className="grid gap-4 md:grid-cols-2">
							{challengesData.completedChallenges.map(ch => (
			...
			{challengesData.activeChallenges.length === 0 && challengesData.completedChallenges.length === 0 && (
