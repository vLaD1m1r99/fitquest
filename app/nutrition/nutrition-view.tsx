...
import { type NutritionDay, type NutritionLog, type Profile } from "@/lib/data"
...
export function NutritionView({
	...
	const getMacroPieData = () => {
		...
		const totalProteinCals = filteredDays.reduce(
			(sum, day) => sum + day.dailyTotals.proteinG * 4,
			0,
		)
		const totalCarbsCals = filteredDays.reduce(
			(sum, day) => sum + day.dailyTotals.carbsG * 4,
			0,
		)
		const totalFatCals = filteredDays.reduce(
			(sum, day) => sum + day.dailyTotals.fatG * 9,
			0,
		)
								...
								<div className="relative w-48 h-48 flex items-center justify-center mb-4">
									...
									<div
										...
										style={{
											...
											transform: `rotate(${
												(todayData.dailyTotals.calories / dailyCalorieTarget) * 360
											}deg)`,
										}}
									></div>
							...
							<div className="flex flex-col justify-center space-y-6">
								<div>
									<div className="flex items-center justify-between mb-2">
										...
										<span className="text-sm text-muted-foreground">
											{Math.round(todayData.dailyTotals.proteinG)}g /{" "}
											{targetMacros.proteinG}g
										</span>
									</div>
									<Progress
										value={Math.min(
								...
								<div>
									<div className="flex items-center justify-between mb-2">
										...
										<span className="text-sm text-muted-foreground">
											{Math.round(todayData.dailyTotals.carbsG)}g /{" "}
											{targetMacros.carbsG}g
										</span>
									</div>
									<Progress
										value={Math.min(
								...
								<div>
									<div className="flex items-center justify-between mb-2">
										...
										<span className="text-sm text-muted-foreground">
											{Math.round(todayData.dailyTotals.fatG)}g /
											{targetMacros.fatG}g
										</span>
									</div>
									<Progress
									...
									<div className="flex items-center justify-between mb-2">
										<div className="flex items-center gap-2">
											<div
												...
												style={{
													backgroundColor: macroColors[macro.name],
												}}
											></div>
											<span className="font-medium text-foreground">
												{macro.name}
