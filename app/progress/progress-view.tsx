...
import { Activity, Brain, Heart, Lightbulb, Moon } from "lucide-react"
...
export function ProgressView({ profile, weightLog, measurementLog, nutritionLog, dailyLog, progressPhotos }: ProgressViewProps) {
	...
	const insights = (() => {
		...
		if (macroAdherence < 50 && filteredNutrition.length > 0)
			result.push("⚠️ Macro adherence is low. Focus on hitting your protein and carb targets consistently.")

		return result
	})()

	const latestMeasurement = filteredMeasurements.length > 0 ? filteredMeasurements[filteredMeasurements.length - 1] : null
						...
						<div className="mt-6">
							...
							<ResponsiveContainer width="100%" height={300}>
								<LineChart data={filteredMeasurements}>
									...
									<Tooltip content={<CustomTooltip />} />
									<Legend />
									<Line type="monotone" dataKey="waistCm" stroke="#a78bfa" dot={{ r: 3 }} name="Waist" isAnimationActive={false} />
									<Line type="monotone" dataKey="hipsCm" stroke="#f472b6" dot={{ r: 3 }} name="Hips" isAnimationActive={false} />
									<Line type="monotone" dataKey="chestCm" stroke="#22d3ee" dot={{ r: 3 }} name="Chest" isAnimationActive={false} />
									<Line type="monotone" dataKey="armsCm" stroke="#fbbf24" dot={{ r: 3 }} name="Arms" isAnimationActive={false} />
									<Line type="monotone" dataKey="thighsCm" stroke="#4ade80" dot={{ r: 3 }} name="Thighs" isAnimationActive={false} />
								</LineChart>
							</ResponsiveContainer>
						</div>
					)}
				...
				<Card className="p-6">
					...
					<ResponsiveContainer width="100%" height={300}>
						<BarChart data={filteredNutrition}>
							...
							<Tooltip content={<CustomTooltip />} />
							<Legend />
							<Bar dataKey="proteinG" stackId="a" fill="#4ade80" name="Protein (g)" isAnimationActive={false} />
							<Bar dataKey="carbsG" stackId="a" fill="#fbbf24" name="Carbs (g)" isAnimationActive={false} />
					...
					<div className="flex items-center gap-4">
						<div className="flex-1">
							...
							<div className="w-full bg-muted rounded-full h-3 overflow-hidden border border-border">
								<div className="bg-accent h-full transition-all" style={{ width: `${macroAdherence}%` }} />
							</div>
						...
						{progressPhotos.photos.map((photo, idx) => (
							<div key={idx} className="bg-muted rounded-lg overflow-hidden">
								...
								<div className="p-3">
									...
									{photo.caption && <p className="text-xs text-muted-foreground mt-1">{photo.caption}</p>}
								</div>
							</div>
						))}
					</div>
